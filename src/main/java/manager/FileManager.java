...
public class FileManager {
    ...
    public static void rewriteFlights(List<Flight> flights) {
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter("flights.txt", false))) {
            ...
            for (Flight f : flights) {
                // flightNum, plane( planeID, planeModel), date, hour, route(dep, arr), departurePlace, arrivalPlace, duration
    ...
    File file = new File(RESERVATION_FILE); 
    if (!file.exists()) return loadedReservations;
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            // Format: resCode;date;flightNum;seatNum;passengerId;bagage 
            if (data.length >= 6) { 
    ...
    try (PrintWriter out = new PrintWriter(new FileWriter(RESERVATION_FILE, true))) { // true => append mode
        ...
        System.err.println("Dosyaya ekleme hatası: " + e.getMessage());
    }
}
